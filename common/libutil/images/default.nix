...
let
  ...
  createBootableMultibootIso =
    ...
    , name ? "${kernel.name}-grub-standalone-x86-legacy-iso"
      # Additional multiboot boot modules.
      # Format: [{file=<derivation or Nix path>; cmdline=<string>;}]
    , bootModules ? [ ]
      # Multiboot version. 1 or 2.
  ...
  createBootableMultibootEfi =
    ...
    , kernelCmdline ? ""
      # Name of the derivation.
    , name ? "${kernel.name}-grub-standalone-x86_64-efi"
      # Additional multiboot boot modules.
      # Format: [{file=<derivation or Nix path>; cmdline=<string>;}]
    , bootModules ? [ ]
      # Multiboot version. 1 or 2.
    ...
    runCommandNoCC name
      ...
      ''
        ...
        grub-mkstandalone \
          ...
          #
          # Further, grub-mkstandalone is a beat. It does not report error when
