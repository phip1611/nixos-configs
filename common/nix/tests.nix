...
in
{
  ...
  kernelboot =
    ...
    {
      testRunQemuDirect =
        bootWithTimeout "iso" "32 bit via MB1"
          "qemu-system-x86_64 -kernel ${bootitem.elf32} ${commonQemuArgs}";
      testRunQemuEfiMb1 =
        bootWithTimeout "efi-mb1" "32 bit via MB1"
          "run-efi --no-common-options --qemu-args='${commonQemuArgs}' ${bootitem.efiMb1}";
      testRunQemuEfiMb2 =
        bootWithTimeout "efi-mb2" "64 bit via MB2"
          "run-efi --no-common-options --qemu-args='${commonQemuArgs}' ${bootitem.efiMb2}";
      testRunQemuIso =
        bootWithTimeout "iso-hybrid-bios" "32 bit via MB2"
          "qemu-system-x86_64 -cdrom ${bootitem.iso} ${commonQemuArgs}";
      ...
      testRunQemuIsoUefi =
        bootWithTimeout "iso-hybrid-uefi" "32 bit via MB2"
          "qemu-system-x86_64 -cdrom ${bootitem.iso} -bios ${pkgs.OVMF.fd}/FV/OVMF.fd ${commonQemuArgs}";
      testRunXenPVH =
        bootWithTimeout "xen-pvh" "32 bit via Xen PVH"
          "cloud-hypervisor --console off --debug-console file=out.txt --kernel ${bootitem.elf64}";
    };
}
